...
import { cards, connect, generate, inspectFilm, inspector, pickStock, prompt, watchToReady } from "./helpers";
...
test("use last frame starts the next shot where the film ended", async ({ page }) => {
  ...
  await watchToReady(cards(page).last());
...
test("reuse settings puts a take's whole setup back in the composer", async ({ page }) => {
  ...
  await page.getByLabel("Frame rate").selectOption("48");
  ...
  await expect(page.getByLabel("Aspect ratio")).toHaveValue("9:16");
  await expect(page.getByLabel("Duration")).toHaveValue("8");
  await expect(page.getByLabel("Frame rate")).toHaveValue("48");
});
...
test("film keys survive a backup, a forget-all and a restore", async ({ page }) => {
  ...
  const [download] = await Promise.all([
    ...
    page.getByRole("button", { name: "Back up keys" }).click(),
  ]);
