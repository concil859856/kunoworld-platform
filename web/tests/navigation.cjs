...
(async () => {
  ...
  try {
    ...
    await page.goto(origin);
    await clickTo(page.locator('.reel-hero').getByRole('link', { name: 'Start creating', exact: false }), '/studio', '#video-prompt');
    await clickTo(page.getByRole('link', { name: 'Back to website', exact: false }), '/');
    await clickTo(page.getByRole('link', { name: 'Try it: LTX-2.5 Pro', exact: true }), '/studio?model=ltx-2.5-pro', '#video-prompt');
    await page.goBack();
    await page.waitForURL(origin + '/');
    await page.goto(origin + '/showcase');
    await clickTo(page.getByRole('link', { name: 'Create from The paper dragon', exact: true }), '/studio?scene=origami-dragon', '#video-prompt');
    await page.waitForFunction(() => document.querySelector('#video-prompt').value.includes('origami dragon'));
