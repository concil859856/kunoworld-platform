...
import type { GoldenManifest, ModelProfile, ModelsResponse, Receipt } from "@kunoworld/sdk";
...
export default function Home() {
  ...
  function chooseModel(id:string,_nextMode?:string){const p=profilesList.find(x=>x.id===id);if(p)composer.actions.setProfile(p);}
  ...
  async function runJob(sub: Submission){
    ...
    try{
      const handle=await client.submit({prompt:sub.prompt,model:sub.requested.id,mode:sub.mode,durationS:composer.state.settings.durationS,resolution:composer.state.settings.resolution,aspectRatio:composer.state.settings.aspectRatio,audio:composer.state.settings.audio,inputs:sub.inputs},stage=>setStatus(({routing:"Finding an available worker…",verifying:"Checking worker evidence…",encrypting:"Encrypting your prompt on this device…",uploading:"Uploading encrypted references…",submitting:"Submitting your encrypted request…"})[stage]));
      setJob(handle);
      if(handle.fallbackReason)setStatus(`Using ${handle.profileId}: ${handle.fallbackReason}`);
      const result=await client.wait(handle,{signal:controller.signal,onProgress:s=>setStatus(`Video ${s.status.replaceAll("_"," ")} · ${handle.profileId}`)});
      const url=URL.createObjectURL(new Blob([new Uint8Array(result.video)],{type:"video/mp4"}));
      const receiptUrl=URL.createObjectURL(new Blob([JSON.stringify(result.receipt,null,2)],{type:"application/json"}));
      resultUrls.current.push(url,receiptUrl);
      setResults(r=>[{id:result.jobId,prompt:sub.prompt,url,receiptUrl,model:result.profileId,receipt:result.receipt},...r]);
      setView("library");
      setStatus("Your video is ready. The result was decrypted and its receipt checked on this device.");
      setJob(null);
    }catch(e){setError(e instanceof Error?e.message:"Generation failed. Please try again.");setStatus("");}finally{setBusy(false);}
  }
